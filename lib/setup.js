...
config.version("0.13.0")
    ...
    .option("--new-scope [file]", "Write a new scope file to the current directory", "scope.json")
    .option("--new-config [file]", "Write a new config file to the current directory", "config.json")
    .option("--new-entry-point [file]", "Write a new entry point file to the current directory", "main.js");
    ...
    connection.then(async session => {
        ...
        let files = config.files.map(async file => {
            ...
            if (config.hooks.initFile) {
                ...
                if (file.toLowerCase().endsWith(".rules.js") || file.toLowerCase().endsWith(".ib.js")) {
                    return context.load(file);
                }
                else {
                    return require(file)(session, context, file);
                }
            }
