...
async function environment(config, hooks) {
    ...
    if (config.hooks.connect) connection = config.hooks.connect(config, session);
    if (connection == null) connection = session(config);
    connection.then(async session => {
        ...
        if (config.verbose) console.log("Building model...");
        let model = await session.model(config.model);
        if (config.hooks.model) await config.hooks.model(model);
        
        if (config.verbose) console.log("Building context...");
        ...
        let files = config.files.map(async file => {
            if (config.verbose) console.log("Loading " + file + "...");
            if (config.hooks.preFile) file = (await config.hooks.preFile(file)) || file;
            if (config.hooks.initFile) config.hooks.initFile(file, context, session);
            else {
                let lower = file.toLowerCase();
                if (lower.endsWith("!.js") || lower.endsWith("$.js")) return context.runInContext(await read(file));
                else {
        ...
        files.reduce((promise, func) => promise.then(result => func().then(Array.prototype.concat.bind(result))), Promise.resolve([])).then(async () => {
            if (config.hooks.files) await config.hooks.files(config.files, context, session);
            ...
            if (config.repl) {
                ...
                if (config.hooks.initRepl) config.hooks.initRepl(context, session);
                else {
            ...
            else if (config.verbose) console.log("Load complete.");
            
            if (config.hooks.ready) config.hooks.ready();
