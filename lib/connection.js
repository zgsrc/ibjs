...
const IB = require('ib'),
      ...
      parseXML = require('fast-xml2js').parseString;
...
exports.Connection = function(options) {
    ...
    function registerCallback(cb, cancel) {
        ...
        var ref = me.callbacks[id] = { callback: cb, cancel: cancel };
        
        ref.timeout = setTimeout(function() {
            var results = ref.results;
            if (ref.cancel) ib[ref.cancel](id);
            unregisterCallback(id);
            cb(new Error("Request timed out."), results);
        }, 5000);
    ...
    function invokeCallback(id, args) {
        ...
        else if (me.options.verbose) {
            console.warn(("IB-SDK warning: No callback registered for request " + id + ".").red);
        }
    }
    ...
    function returnResults(id) {
        if (me.callbacks[id]) {
            ...
            console.debug(("IB-SDK warning: No callback registered for request " + id + ".").red);
        }
    }
    ...
    var ib = me.socket = new IB({
        clientId: me.options.client || exports.client++,
        ...
        if (args && args.id && args.id > 0) invokeCallback(args.id, [ err, null ]);
    ...
    this.setMarketDataType = function(marketDataType) {
    ...
    this.details = function(contract, cb) {
    ...
    this.bar = function(contract, options, cb) {
        ...
        if (!options.regularTradingHours) {
            options.regularTradingHours = false;
        }
    ...
    // QUOTES
    ////////////////////////////////////////////////////////////////////////
    this.ticker = function(contract, fields, cb) {
    ...
    // L2 QUOTES
    ////////////////////////////////////////////////////////////////////////
    this.marketDepth = function(contract, rows, cb) {
        if (cb == null && Object.isFunction(rows)) {
            cb = rows;
            rows = 10;
        }
        
        ib.reqMktDepth(registerCallback(cb, "cancelMktDepth"), contract, rows);
    };
    ...
    // SCANNERS
    ////////////////////////////////////////////////////////////////////////
    this.scannerParameters = function(cb) {
    ...
    // TRADE HISTORY
    ////////////////////////////////////////////////////////////////////////
    this.executions = function(options, cb) {
        if (cb == null && Object.isFunction(options)) {
            cb = options;
            options = null;
        }
    ...
    this.news = function(all, cb) {
        if (cb == null && Object.isFunction(all)) {
            cb = all;
            all = true;
        }
...
exports.connect = function(options, cb) {
    ...
    var connection = new exports.Connection(options);
